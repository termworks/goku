...
- full-size letter bodies with shortened descenders so `g`, `j`, `p`, `q`, and
  `y` no longer meet the bottom cell edge;
- optically centered wide and Bold letters, including `O`, with a conservative
  half-source-pixel horizontal outline margin;
- protected accents at the top edge, while cell-spanning em dashes remain
...
- unchanged native Gohu proportions with a finer 3x pixel grid for imported
  Unicode symbols and Nerd icons;
- programming operators remain separate characters with no substitutions;
